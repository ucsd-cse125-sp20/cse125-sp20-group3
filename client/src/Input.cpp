...
int Input::EncodeToBuf(char buf[])
{
	...
	int buildConfirm = inputs[INPUT_ACTION_3] ? BUILD_CONFIRM : (inputs[INPUT_ACTION_2] ? BUILD_CANCEL : 0); //f to confirm, r to cancel
	bool harvestResource = inputs[INPUT_ACTION_1] != 0.0f; //e to harvest resource player is looking at

	printf("type: %c confirm: %d harvest: %i\n", buildType, buildConfirm, harvestResource);
