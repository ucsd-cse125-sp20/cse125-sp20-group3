...
StructuredBuffer<float4x4> modelToSceneMatrices : register(t0, UPDATE_FREQ_NONE);
...
PsIn main(VsIn In)
{
	float4x4 modelToScene = modelToSceneMatrices[nodeIndex];
	float4x4 modelToWorld = mul(toWorld, modelToScene);
